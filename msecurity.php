...
function msecurity_options_page() {
    $options = get_option('msecurity_options', array());
...
function msecurity_settings_init() {
    register_setting('msecurity_options', 'msecurity_options', 'msecurity_options_validate');
...
function msecurity_api_secret_key_render() {
    $options = get_option('msecurity_options', array());
    $value = isset($options['msecurity_api_secret_key']) ? esc_attr($options['msecurity_api_secret_key']) : '';
    ?>
    <input type='text' name='msecurity_options[msecurity_api_secret_key]' value='<?php echo $value; ?>'>
    <?php
}
...
function msecurity_api_public_key_render() {
    $options = get_option('msecurity_options', array());
    $value = isset($options['msecurity_api_public_key']) ? esc_attr($options['msecurity_api_public_key']) : '';
    ?>
    <input type='text' name='msecurity_options[msecurity_api_public_key]' value='<?php echo $value; ?>'>
    <?php
}
...
function msecurity_commission_percentage_render() {
    $options = get_option('msecurity_options', array());
    $value = isset($options['msecurity_commission_percentage']) ? esc_attr($options['msecurity_commission_percentage']) : '';
    ?>
    <input type='number' name='msecurity_options[msecurity_commission_percentage]' value='<?php echo $value; ?>' step='0.01'>
    <?php
}
...
function msecurity_import_products() {
    $options = get_option('msecurity_options', array());
    $secret_key = sanitize_text_field($options['msecurity_api_secret_key']);
    $public_key = sanitize_text_field($options['msecurity_api_public_key']);
        ...
        if ($data['success']) {
            foreach ($data['platform'] as $service) {
                ...
                if ($product_id) {
                    ...
                    update_post_meta($product_id, '_regular_price', sanitize_text_field($cost));
                    update_post_meta($product_id, '_price', sanitize_text_field($cost)); // Ensure price is set
                    update_post_meta($product_id, '_sku', sanitize_text_field($service['sku']));
                    update_post_meta($product_id, '_virtual', 'no');
                    update_post_meta($product_id, '_downloadable', 'yes');
                    update_post_meta($product_id, '_stock_status', 'instock');

                    // Set the product image
                    $image_url = esc_url($service['image']);
...
function msecurity_upload_image($image_url) {
    ...
    $attachment_id = wp_insert_attachment(array(
        'guid'           => esc_url_raw($sideload['url']),
        'post_mime_type' => sanitize_mime_type($sideload['type']),
...
function msecurity_dashboard_widget_render() {
    ...
    if (is_wp_error($response)) {
        ...
        if (isset($data['balance'])) {
            echo '<div class="msecurity-balance-widget"><h2>Balance: ' . esc_html($data['balance']) . '</h2></div>';
        } else {
...
function msecurity_purchase_product($email, $sku, $reference) {
    ...
    if (is_wp_error($balance_response)) {
        return array('error' => 'Error: ' . esc_html($balance_response->get_error_message()));
    } else {
        ...
        if (isset($balance_data['balance']) && $balance_data['balance'] > 0) {
            ...
            if (is_wp_error($purchase_response)) {
                return array('error' => 'Error: ' . esc_html($purchase_response->get_error_message()));
            } else {
                ...
                if (isset($purchase_data['message']) && $purchase_data['message'] === 'License purchased successfully.') {
                    return array('success' => true, 'message' => esc_html($purchase_data['message']), 'license' => sanitize_text_field($purchase_data['license']));
                } else {
...
function msecurity_add_license_key_to_order($order_id, $email, $sku, $reference) {
    ...
    if (isset($purchase_response['success'])) {
        ...
        $order->add_order_note('License Key: ' . esc_html($purchase_response['license']));
        update_post_meta($order_id, '_license_key', sanitize_text_field($purchase_response['license']));
    } else {
        // Handle error
        $order = wc_get_order($order_id);
        $order->add_order_note('Error: ' . esc_html($purchase_response['error']));
        $order->update_status('pending', 'Error: ' . esc_html($purchase_response['error']));
    }
}
...
function msecurity_process_woocommerce_order($order_id) {
    ...
    if ($order->get_status() === 'completed') {
        ...
        foreach ($items as $item) {
            ...
            msecurity_add_license_key_to_order($order_id, $email, $sku, $reference);
        }
    }
}
...
function msecurity_options_validate($input) {
    $output = array();
